...
const blocks = [
  ...
  ["hormone_sensitive_and_nmcrpc", 4],
...
try {
  ...
  await mobile.goto(`${baseUrl}/parcours/prostate/foundations`, { waitUntil: "domcontentloaded" });
  await mobile.locator(".orientationView").waitFor();
  await mobile.screenshot({ path: "artifacts/ui-audit/foundations-mobile.png", fullPage: true });
  await mobile.close();

  console.log("Audit interface réussi : accueil, 15 blocs, 5 blocs avancés, mode RCP et mobile.");
} finally {
  await browser.close();
}
